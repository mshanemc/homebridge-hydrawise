...
export class HydrawiseSprinkler {
  ...
  constructor(zone: HydrawiseZone, platform: HydrawisePlatform) {
    ...
    let existingAccessory = that.platform.accessories.find(
    ...
    if (existingAccessory !== undefined) {
      ...
      that.platform.api.registerPlatformAccessories(
        PLUGIN_NAME,
        PLATFORM_NAME,
        [that.accessory]
      );
      ...
      service = that.accessory.addService(
        that.platform.api.hap.Service.Valve,
        'Sprinkler'
      );
      service.setCharacteristic(
        that.platform.api.hap.Characteristic.ValveType,
        '1'
      );
    }
    ...
    service
      ...
      .updateValue(that.zone.isRunning);
    service
      .getCharacteristic(that.platform.api.hap.Characteristic.InUse)
      .updateValue(that.zone.isRunning);
    service
      .getCharacteristic(that.platform.api.hap.Characteristic.RemainingDuration)
      .updateValue(that.zone.remainingRunningTime);
    ...
    service
      ...
      .on(
        CharacteristicEventTypes.SET,
        (value: CharacteristicValue, callback: CharacteristicSetCallback) => {
          that.platform.debug(`duration requested: ${value}`);
          that.zone
            .run((value as number) / 60)
            .then((data) => {
              that.platform.log.info(
                'Set Duration for ' + zone.name + ' to ' + value
              );
              callback();
            })
            .catch((error) => {
  ...
  update(zone: HydrawiseZone): void {
    let service = this.accessory.getService(
      this.platform.api.hap.Service.Valve
    )!;
    ...
    if (this.zone.remainingRunningTime != zone.remainingRunningTime) {
      // Log
      this.platform.log.info(
        'Received new remaining duration for ' +
          zone.name +
          ': ' +
          zone.remainingRunningTime
      );
      ...
      service
        ...
        .updateValue(zone.remainingRunningTime);
  ...
  unregister(): void {
    ...
    // Log
    that.platform.log.info(
      'Unregistering sprinkler accessory for deleted Hydrawise zone: %s',
      that.zone.name
    );

    // Remove
    that.platform.api.unregisterPlatformAccessories(
      PLUGIN_NAME,
      PLATFORM_NAME,
      [that.accessory]
    );
    that.platform.accessories = that.platform.accessories.filter(
      (item: PlatformAccessory) => item !== that.accessory
    );
  }
}
