...
export class HydrawisePlatform implements DynamicPlatformPlugin {
        ...
        .then((controllers: HydrawiseController[]) => {
          ...
          if (controllers.length > 0) {
            ...
            if (
              ...
              if (this.hydrawise.type == HydrawiseConnectionType.LOCAL) {
                ...
                that.pollingInterval =
                  DEFAULT_POLLING_INTERVAL_CLOUD * controllers.length;
              }
            }
            ...
            controllers.map((controller: HydrawiseController) => {
              ...
              // Continuous updates of the zones
              setInterval(() => {
                that.getZones(controller);
              }, that.pollingInterval);
            });
  ...
  getZones(controller: HydrawiseController): void {
    ...
    toCheckSprinklers = toCheckSprinklers.filter(
      (item: HydrawiseSprinkler) => item.zone.controller.id == controller.id
    );
    ...
    controller
      ...
      .then((zones: HydrawiseZone[]) => {
        ...
        zones.map((zone: HydrawiseZone) => {
          ...
          let existingSprinkler = that.sprinklers.find(
            (x) => x.zone.relayID == zone.relayID
          );
          ...
          if (existingSprinkler !== undefined) {
            ...
            toCheckSprinklers = toCheckSprinklers.filter(
              (item: HydrawiseSprinkler) => item.zone.relayID !== zone.relayID
            );
          }
          // Sprinkler does not exist yet
          else {
            // Log
            that.log.debug(
        ...
        toCheckSprinklers.map((sprinkler) => {
          ...
          that.sprinklers = that.sprinklers.filter(
            (item: HydrawiseSprinkler) => item !== sprinkler
          );
        });
      })
